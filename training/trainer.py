...
class StyleGAN2Trainer:
    ...
    def __init__(self, 
        ...
        self.optimizer_G = torch.optim.Adam(self.G.parameters(), lr=g_lr, betas=betas)
        self.optimizer_D = torch.optim.Adam(self.D.parameters(), lr=d_lr, betas=betas)
        ...
        self.aug_p = 0.0  # Start with no augmentation
        ...
        # Fixed latents for visualization
    ...
    def train_discriminator(self, real_imgs):
        ...
        with autocast(enabled=self.use_amp):
            ...
            d_loss = F.softplus(fake_scores).mean() + F.softplus(-real_scores).mean()
        
        # R1 regularization (every 16 iterations)
        if self.iteration % 16 == 0:
            aug_real.requires_grad_(True)
    ...
    def update_ada(self, real_scores):
        """Update adaptive augmentation probability"""
    ...
    def generate_samples(self, iteration):
        ...
        if self.config['logging']['use_wandb']:
            wandb.log({'samples': wandb.Image(str(save_path))}, step=iteration)
